(function () {
        ...
        chrome.runtime.onMessage.addListener((msg, _sender, reply) => {
            ...
            switch (msg.type) {
                ...
                case 'YTMS_GET_METERS': {
                    ...
                    const v = engine.isAttached() ? engine.getMediaElement() : document.querySelector('video');
    ...
    async function boot() {
        ...
        if ('mediaSession' in navigator) {
            navigator.mediaSession.onmetadatachange = onPossibleTrackChange;
        }

        document.addEventListener('loadedmetadata', (e) => {
            if (e.target && e.target.tagName === 'VIDEO') onPossibleTrackChange();
        }, true);
    ...
    function onPossibleTrackChange() {
        ...
        if (vid && vid !== _lastVideoId) {
            _lastVideoId = vid;
            engine.resetForTrackChange();
            log('Track change detected — reset.');
        }
    }
